...
-- [functionality v1.1] Place given cycle in homology algebra.
...
-- This function takes the list of cycles and returns them all in HA as a list of elements
-- The list should all have elements of the same degree, and all be in A.natural.
substituteInHomology = method(TypicalValue=>List,Options=>{Verbosity=>0})
substituteInHomology (DGAlgebra, List) := opts -> (A, cycleList) -> (
  -- check to see if the cycles are all in the same degree
  cycleDegree := first degree first cycleList;
  if not all(cycleList, z -> first degree z === cycleDegree) then error "List of cycles must all be of the same degree.";
  -- this function makes it where the code should work for inhomogeneous DGAs
  firstMaybe := d -> if isHomogeneous A then first d else d;
 
  R := A.ring;
  maxGenDeg := maxDegree A;
  maxRelDeg := infinity;
  if maxGenDeg =!= infinity then (maxGenDeg = cycleDegree; maxRelDeg = 2*cycleDegree;);
  HA := homologyAlgebra(A,GenDegreeLimit=>maxGenDeg, RelDegreeLimit=>maxRelDeg);

  -- set up ring maps
  HAToA := map(A.natural,HA,HA.cache.cycles);
  AToR := map(R,A.natural);
  RToH0A := map(zerothHomology A,R); 

  cycleDegrees := if isHomogeneous A then cycleList / degree else cycleList / degree / first;
  degreeTable := hashTable(join, pack(2,mingle {cycleDegrees,apply(cycleList, z -> {z})}));
  ...
  imageList := flatten apply(pairs degreeTable, (cycleGroupDegree,cycleGroup) -> (
                    haBasis := flatten entries basis(cycleGroupDegree,HA);
                    if haBasis =!= {} then (
                      aBasis := flatten entries getBasis(firstMaybe cycleGroupDegree,A);
                      thisDiff := polyDifferential((firstMaybe cycleGroupDegree)+1,A);
	              cycleTargetCoeff := last coefficients(matrix{haBasis / HAToA},Monomials=>aBasis);
	              cycleGroupCoeff := last coefficients(matrix{cycleGroup},Monomials=>aBasis);
	              -- the following commands fix the degrees since function application does not seem to maintain them.
	              cycleTargetCoeff = map(target thisDiff, R^(apply(degrees source cycleTargetCoeff, d -> drop(d,1))), AToR(cycleTargetCoeff));
	              cycleGroupCoeff = map(target thisDiff, R^(apply(degrees source cycleGroupCoeff, d -> drop(d,1))), AToR(cycleGroupCoeff));
	              ...
	              imageHACoeffs := RToH0A((cycleGroupCoeff // hGB)^{0..(#haBasis)-1});
	              flatten entries (basis(cycleGroupDegree,HA)*imageHACoeffs)
                    )
                    else apply(cycleGroup, z -> 0_HA)
	         )
	       );
  imageList
)

substituteInHomology (DGAlgebra, RingElement) := opts -> (A, z) -> first substituteInHomology (A,{z})

TEST ///
restart
needsPackage "DGAlgebras"
R = QQ[a,b,c,d]/ideal{a^3,b^3,c^3,d^3};
KR = koszulComplexDGA R
HKR = HH KR
gensR = gens R;
gensKR = gens KR.natural;
-- pain in the arse to get this element in the right ring!
z1 = (gensR#0)^2*(gensKR#0)
z2 = (gensR#0)*(gensKR#1) - (gensR#1)*(gensKR#0)
z3 = (gensR#0)^2*(gensR#1)^2*(gensKR#0)*(gensKR#1)
z4 = (gensR#2)^2*(gensKR#2)
substituteInHomology(KR,{z1,z2})
h1 = substituteInHomology(KR,z1)
h2 = substituteInHomology(KR,z2)
h3 = substituteInHomology(KR,z3)
h4 = substituteInHomology(KR,z4)
h14 = substituteInHomology(KR,z1*z4)
HKR.cache.cycles
HKRToKR = map(KR.natural,HKR,matrix{HKR.cache.cycles})
HKRToKR h14
z1*z4

restart
debug needsPackage "DGAlgebras"
S = QQ[x,y,z]/ideal{x^3-y^2,z^3};
KS = koszulComplexDGA S
gensS = gens S;
gensKS = gens KS.natural;
-- pain in the arse to get this element in the right ring!
z1 = (gensS#0)^2*(gensKS#0)-(gensS#1)*(gensKS#1)
z2 = (gensS#0)*(gensKS#1) - (gensS#1)*(gensKS#0)
substituteInHomology(KS,{z1,z2})
substituteInHomology(KS,z1)
substituteInHomology(KS,z2)
///
...
-- It returns a hash table which, given teh 'tensor monomial' gives the element of Massey operation for that monomial.
...
homology DGAlgebraMap := opts -> f -> (
   ...
   imageHash := hashTable flatten apply(pairs degreeTable, (cycleDegree,cycleImageGroup) -> (
                              images := substituteInHomology(B,cycleImageGroup / last);
			      pack(2, mingle {apply(cycleImageGroup, z -> first z), images})
			   )
		        ); 
