...
document {
  ...
  SYNOPSIS {
    Headline => "the next subset after a given one",
    Usage => "nextSubset(n,S)",
    BaseFunction => "nextSubset",
    Inputs => {
  ...
  SYNOPSIS {
    Headline => "the subset before a given one",
    Usage => "prevSubset(n,S)",
    BaseFunction => "prevSubset",
    Inputs => {
      "n" => ZZ => "the size of the containing set",
      "S" => List => "list of elements of a subset of {0..n-1}",
    },
    Outputs => {
      List => "the subset before S"
    },
    PARA {
  ...
  PARA {
    "If S is the last subset (the whole set {0..n-1}) then nextSubset(n,S)
    returns null and subsequent calls continue to return null.
    If S is the first subset then prevSubset(n,S)
    returns null and subsequent calls continue to return null."
  },
  
  EXAMPLE lines ///
    Z = {0,1,2,3,4}; -- the last subset
    Z = nextSubset(5,Z) -- now Z === null
    Z = nextSubset(5,Z) -- still Z === null
    A = {}; -- the first subset
    A = prevSubset(5,Z) -- now A === null
    A = prevSubset(5,Z) -- still A === null
  ///,
